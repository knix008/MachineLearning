...
def generate_image(
    ...
    try:
        ...
        for i, img in enumerate(images):
            ...
            output_path = f"{base_name}_{timestamp}_{device_name}_step{steps}_cfg{guidance_scale}_tcfg{true_cfg_scale}_{int(width)}x{int(height)}_seed{int(seed)}{suffix}.png"
        ...
        return (
            images,
            f"✓ 이미지 생성 완료! ({elapsed:.1f}초) 저장됨: {', '.join(saved_paths)}",
        )
        ...
        with gr.Row():
            ...
            with gr.Column():
                gallery_output = gr.Gallery(
                    label="생성된 이미지", columns=2, height=800
                )
