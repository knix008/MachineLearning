...
# Outfit
DEFAULT_OUTFIT = "She is wearing a loose, flowy white sundress with an off-shoulder asymmetric neckline — one shoulder is fully covered by the dress strap while the other shoulder is completely bare and exposed. The dress features a deep side slit that opens all the way up to the thigh, fully revealing one bare thigh as she walks. A slim black leather belt is cinched at her waist, accentuating her figure. The lightweight breezy fabric gently billows in the sea wind. The dress drapes softly over her figure, casual and relaxed beach style."

# Pose & Action
DEFAULT_POSE = "She is walking gracefully along the beach shoreline, mid-stride with natural movement. One leg is fully extended forward while the other steps back, with one leg exposed from the thigh down as the dress hem is lifted by the breeze on that side. Her posture is relaxed and carefree, arms swinging naturally at her sides. She glances slightly toward the camera with a soft, natural expression."
...
with gr.Blocks(title="FLUX.1-dev Text-to-Image", js=_BLOCK_CTRL_S_JS) as demo:
...
if __name__ == "__main__":
    try:
        ...
        print("\nKeyboardInterrupt detected. Releasing resources...")
    finally:
        demo.close()
        if "pipe" in globals():
            del pipe
