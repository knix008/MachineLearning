...
# NOTE: As of June 2024, YOLO 11 does not exist. This uses YOLOv8 (Ultralytics) as the latest available.
from ultralytics import YOLO

# Load the YOLOv8 model (replace with your own model if available)
model = YOLO("yolov8n.pt")  # Use a different checkpoint if desired
...
def detect_objects(pil_img):
    ...
    for box in results[0].boxes:
        ...
        label = results[0].names[cls_id] if hasattr(results[0], "names") and cls_id != -1 else str(cls_id)
...
demo = gr.Interface(
    ...
    title="YOLO Object Detection (YOLOv8)",
