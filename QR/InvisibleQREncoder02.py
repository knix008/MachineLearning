...
def extract_qr_from_image_file(stego_img):
    ...
    if data:
        return qr_mask_img, f"Decoded QR data: {data}"
    else:
        return qr_mask_img, "QR code could not be decoded. Try a different image or adjust parameters."
...
with gr.Blocks() as demo:
    ...
    with gr.Tab("Extract QR"):
        ...
        with gr.Row():
            ...
            # 오른쪽 열: 추출 QR, 텍스트
            with gr.Column():
                qr_mask_preview = gr.Image(label="추출된 QR 코드", type="pil")
                qr_text_out = gr.Textbox(label="Decoded QR Data")
        def extract_and_preview(img):
            if img is None:
                return None, ""
            qr_mask_img, text = extract_qr_from_image_file(img)
            return qr_mask_img, text
        extract_btn.click(
            fn=extract_and_preview,
            inputs=extract_file,
            outputs=[qr_mask_preview, qr_text_out]
        )
