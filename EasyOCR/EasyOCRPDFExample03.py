...
def process_file(input_file):
    ...
    if hasattr(input_file, "name") and input_file.name.lower().endswith(".pdf"):
        ...
        print(
            f">> Processing time for PDF page: {datetime.timedelta(seconds=end - start)}"
        )
    else:
        ...
        print(
            f">> Processing time for image: {datetime.timedelta(seconds=end - start)}"
        )
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column():
            input_file = gr.File(
                label="Upload Image or PDF", file_types=["image", ".pdf"]
            )
    ...
    run_btn.click(
        process_file,
        inputs=input_file,
        outputs=[input_display_display, output_image, output_text],
    )
