...
SUBJECT = "A photography of a beautiful young skinny Korean woman with a soft idol aesthetic, lying on a sun lounger beside a swimming pool."

FOOT = "Both feet resting flat on the sun lounger, relaxed, entire feet fully visible."

LEG = "Both legs fully straight and relaxed, resting flat on the sun lounger from hip to heel, legs together, no gap between legs."

FACE = "She has a fair, clear complexion. She is wearing blue contact lenses that contrast with her dark hair. Her expression is innocent and curious, looking directly at the camera. She has long, voluminous wavy jet-black hair."

BODY = "She is naked, lying flat on her back on the sun lounger, body fully stretched out, slim figure visible from directly above."

ARM = "Both arms raised up beside her head, elbows bent outward, resting naturally on the lounger."

HAND = "Both hands raised behind her head, fingers gently gripping and gathering her wet hair behind her head."

FOOTWEAR = ""

LEGWEAR = ""

BOTTOM = "lot of pubic hair visible, natural and untrimmed, covering the mons pubis and labia, wet and clumped together, dark black color, contrasts with her fair skin tone."
...
HEAD = "Head centered, face looking directly up toward the camera overhead, hair spread naturally around her head."

SETTING = "Luxury outdoor swimming pool, turquoise blue water visible beside the sun lounger, white pool deck, poolside lounge chair, tropical atmosphere."

LIGHTING = "Bright warm sunny daylight directly overhead, strong natural sunlight, soft shadows, vibrant colors, clear summer day."

CAMERA = "Directly overhead top-down aerial view, full body in frame from head to toe, high-fidelity skin textures."

POSITIVE = "8k, perfect anatomy, Photorealistic, overhead shot, bird's eye view, top-down perspective, ten fingers, ten toes, well formed fingers and toes."

NEGATIVE = "Extra hands, extra legs, extra feet, extra arms, waist pleats, paintings, sketches, worst quality, low quality, normal quality, low resolution, monochrome, grayscale, skin spots, wet, acnes, skin blemishes, age spot, backlight, mutated hands, poorly drawn hands, blurry, bad anatomy, bad proportions, extra limbs, disfigured, missing arms, extra legs, fused fingers, too many fingers, unclear eyes, bad hands, missing fingers, extra digit, bad hands, missing fingers."
...
def load_model(device_name=None):
    """Load FLUX.1-dev with uncensored LoRA. CLIP disabled — T5-XXL only."""
...
def generate_image(
    ...
    try:
        ...
        if clip_max_len is None and hasattr(pipe, "tokenizer") and pipe.tokenizer is not None:
        ...
        raw_clip_ids = pipe.tokenizer(prompt_clip, truncation=False, return_tensors="pt")["input_ids"][0]
        ...
        # ---- T5 token count ----
        raw_ids = pipe.tokenizer_2(prompt_t5, truncation=False, return_tensors="pt")["input_ids"][0]
        ...
        if true_cfg_scale > 1.0 and negative and negative.strip():
            ...
            neg_clip_inputs = pipe.tokenizer_2(
                negative,
                padding="max_length",
                max_length=max_len,
                truncation=True,
            ...
            pipe_kwargs["negative_pooled_prompt_embeds"] = neg_clip_out.pooler_output.to(dtype=DTYPE)
...
def main():
    ...
    with gr.Blocks(title="Flux.1-dev Uncensored Text-to-Image Generator") as interface:
        gr.Markdown("# Flux.1-dev Uncensored Text-to-Image Generator")
        gr.Markdown(
            f"AI를 사용하여 텍스트에서 이미지를 생성합니다."
            f" (Device: **{DEVICE.upper()}**)\n\n"
            f"> **T5-XXL 단일 인코더** 사용. CLIP 비활성화."
        )
            ...
            with gr.Column(scale=1):
                ...
                with gr.Accordion("프롬프트 섹션 (Subject · 포즈 · 의상 · 배경 · 조명 · 카메라)", open=True):
                    prompt_subject = gr.Textbox(
                        ...
                        value=SUBJECT,
                        lines=2,
                        placeholder="예: a beautiful young woman posing",
                    ...
                    prompt_leg = gr.Textbox(
                        ...
                        value=LEG,
                        lines=1,
                        placeholder="예: legs slightly apart, knees bent",
                        info="다리 자세를 설명합니다.",
                    )
                    prompt_face = gr.Textbox(
                        ...
                        value=FACE,
                        lines=2,
                        placeholder="예: fair complexion, soft expression",
                    ...
                    prompt_body = gr.Textbox(
                        ...
                        value=BODY,
                        lines=2,
                        placeholder="예: body facing camera, relaxed posture",
                    ...
                    prompt_arm = gr.Textbox(
                        ...
                        value=ARM,
                        lines=1,
                        placeholder="예: arms relaxed at sides",
                    ...
                    prompt_hand = gr.Textbox(
                        ...
                        value=HAND,
                        lines=1,
                        placeholder="예: hands resting gently",
                    ...
                    prompt_footwear = gr.Textbox(
                        ...
                        value=FOOTWEAR,
                        lines=1,
                        placeholder="예: barefoot, high heels",
                        info="신발 등을 설명합니다.",
                    )
                    prompt_legwear = gr.Textbox(
                        ...
                        value=LEGWEAR,
                        lines=1,
                        placeholder="예: sheer stockings",
                        info="스타킹, 양말 등을 설명합니다.",
                    )
                    prompt_bottom = gr.Textbox(
                        label="10. 하의 (Bottom)",
                        value=BOTTOM,
                        lines=2,
                        placeholder="예: mini skirt",
                        info="하의 등을 설명합니다.",
                    )
                    ...
                    prompt_headwear = gr.Textbox(
                        ...
                        value=HEADWEAR,
                        lines=1,
                        placeholder="예: floral hairpin",
                        info="모자, 헤어핀 등을 설명합니다.",
                    )
                    prompt_armwear = gr.Textbox(
                        label="13. 팔 장식 (Armwear)",
                        value=ARMWEAR,
                        lines=1,
                        placeholder="예: silver bracelet",
                        info="팔찌 등을 설명합니다.",
                    )
                    prompt_head = gr.Textbox(
                        ...
                        value=HEAD,
                        lines=1,
                        placeholder="예: head tilted slightly",
                    ...
                    prompt_setting = gr.Textbox(
                        ...
                        value=SETTING,
                        lines=2,
                        placeholder="예: bedroom, studio, outdoor",
                        info="배경, 장소, 환경 등을 설명합니다.",
                    )
                    prompt_lighting = gr.Textbox(
                        label="16. 조명 (Lighting)",
                        value=LIGHTING,
                        lines=2,
                        placeholder="예: soft natural light, studio lighting",
                        info="조명 조건, 분위기를 설명합니다.",
                    )
                    prompt_camera = gr.Textbox(
                        label="17. 카메라 설정 (Camera)",
                        value=CAMERA,
                        lines=2,
                        placeholder="예: 8k, sharp focus, soft bokeh",
            ...
            with gr.Column(scale=1):
                ...
                with gr.Row():
                    guidance_scale = gr.Slider(
                        ...
                        info="프롬프트 준수도. 낮으면 창의적, 높으면 정확. 권장: 3.5~8",
                    )
                ...
                with gr.Row():
                    ...
                    max_sequence_length = gr.Slider(
                        ...
                        info="T5 인코더 최대 길이. 긴 프롬프트는 높은 값 필요.",
                    )
                with gr.Row():
