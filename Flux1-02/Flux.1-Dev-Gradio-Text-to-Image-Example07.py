...
DEFAULT_ANATOMY = "Five fingers on each hands."
...
DEFAULT_OUTFIT = "She wears an extremely tiny micro pink string bikini top, the triangles are the smallest possible size barely covering only the nipples, thin string ties, plain smooth fabric, absolute maximum skin exposure on chest and torso. Extremely tiny micro pink string bikini bottom, the smallest possible size barely covering only the groin, thin string sides, plain smooth fabric, exposing entire stomach, hips, thighs fully. Headwear: tall, upright blue fabric bunny ears with plain white inner lining and a plain white headband, accented with a small white bow."
DEFAULT_POSE = "Lying on the bed, full body extended lengthwise. Body relaxed and natural. One arm resting loosely at the side, one hand placed naturally flat on the bed surface. Other arm resting gently, other hand lightly placed on the stomach. Legs slightly bent at the knees, ankles crossed. Eyes glancing shyly upward toward camera with a faint blush expression. Full body visible head to toe."
...
def combine_prompt_sections(
    anatomy, appearance, outfit, pose, setting, lighting, camera
):
    """Combine separate prompt sections into one final prompt string."""
    sections = [anatomy, appearance, outfit, pose, setting, lighting, camera]
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                prompt_anatomy = gr.Textbox(
                    ...
                    lines=2,
                    placeholder="예: Five fingers on each hands.",
                    info="신체 구조 관련 보정 키워드입니다.",
                )
