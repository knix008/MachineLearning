...
# Anatomy
DEFAULT_ANATOMY = "Perfect anatomy, no extra finger, no missing fingers, no deformed fingers, no fused fingers, perfect hand structure, perfect feet structure, perfect body proportion, no extra hands, no extra feet, no distorted body."
...
# Outfit
DEFAULT_OUTFIT = "She is wearing only a black bra and black thong panties, nothing else. A simple black bra covering her chest and matching black thong with thin minimal straps along the sides, plain smooth fabric with no lace. No other clothing, no stockings, no accessories."

# Pose & Action
DEFAULT_POSE = "She is lying flat on a white bed, fully relaxed natural pose. One arm resting naturally with the hand placed softly on her stomach, palm down on her belly. The other arm bent slightly upward, with the index finger of that hand gently and lightly touching her lips in a soft playful gesture. Her legs are little bit opend. One leg extended straight, the other leg slightly bent at the knee in a natural relaxed position. Entire full body visible from head to toe within the frame, nothing cropped out. Looking up at the camera above."

# Background & Setting
DEFAULT_BACKGROUND = "Clean white bed with white sheets and white pillows. Minimal bedroom interior. White bedding, soft cotton fabric texture."

# Lighting
DEFAULT_LIGHTING = "Bright, well-lit room with abundant natural daylight. Bright overhead lighting, clean and evenly lit. Bright white ambient light flooding the room, no harsh shadows, crisp and clear illumination on her skin and the white bedding."

# Camera & Shot Style
DEFAULT_CAMERA = "Directly overhead top-down bird's eye view, camera positioned straight above looking straight down at the subject lying on the bed. Full body symmetrically visible from directly above. 24mm wide lens, everything in sharp focus. Realistic lifestyle indoor photography, aerial perspective."
...
with gr.Blocks(title="FLUX.1-dev Text-to-Image") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            gr.Markdown("### Output")
            output_image = gr.Image(label="Output Image", type="pil", height=700)
