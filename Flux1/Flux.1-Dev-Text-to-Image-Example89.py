...
LEG = "Even weight on both legs; thighs and knees forward, feet under hips."
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                gr.Markdown(
                    ...
                    "- **T5**: Subject → 포즈·의상·머리 → Setting → Camera → Lighting → Positive 순.\n"
                ...
                with gr.Accordion("CLIP 프롬프트 (Subject만)", open=True):
                ...
                with gr.Accordion("T5 프롬프트 (전체 섹션 결합)", open=True):
                    prompt_t5 = gr.Textbox(
                        ...
                        interactive=True,
                        info=(
                            "Subject → 포즈·의상·머리 → Setting → Camera → Lighting → Positive 순으로 자동 결합. "
            ...
            with gr.Column(scale=1):
                ...
                output_grid = gr.Image(label="생성된 이미지 (전체 보기)", height=420)
