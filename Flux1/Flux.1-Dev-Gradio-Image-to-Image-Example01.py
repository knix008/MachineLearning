...
def load_model(device_name=None):
    ...
    print("FluxImg2ImgPipeline 로드 중... T5-XXL 텍스트 인코더만 사용합니다. (CLIP 비활성화)")
...
def generate_image(
    ...
    try:
        ...
        print(f"입력 이미지 크기 조정: {input_image.size} → ({int(width)}, {int(height)})")
            ...
            with gr.Column(scale=1):
                ...
                with gr.Row():
                    width = gr.Slider(
                        ...
                        step=64,
                        value=768,
                        info="출력 이미지 너비 (픽셀). 입력 이미지 업로드 시 자동 설정됩니다.",
                    )
                    height = gr.Slider(
                        label="출력 높이",
                        minimum=256,
                        ...
                        step=64,
                        value=1536,
                        info="출력 이미지 높이 (픽셀). 입력 이미지 업로드 시 자동 설정됩니다.",
                    )
                ...
                with gr.Row():
                    ...
                    strength = gr.Slider(
                        ...
                        step=0.01,
                        value=0.75,
                        info="낮을수록 원본 이미지 유지, 높을수록 프롬프트 방향으로 크게 변형. 권장: 0.5-0.85",
                    )
