...
def print_system_info():
    ...
    if cpu_freq:
        print(f"  최대 주파수: {cpu_freq.max:.2f} MHz")
        print(f"  현재 주파수: {cpu_freq.current:.2f} MHz")
    
    # 메모리 정보
    print(f"\n[메모리]")
    mem = psutil.virtual_memory()
    print(f"  총 RAM: {mem.total / (1024**3):.2f} GB")
    ...
    if torch.cuda.is_available():
        ...
        print(f"  Metal Performance Shaders 사용 가능")
    else:
        print(f"  디바이스: CPU")
...
def cleanup_resources():
    ...
    print(f"\n[종료 전 메모리 상태]")
    print(f"  사용 중: {mem_before.used / (1024**3):.2f} GB ({mem_before.percent}%)")
    ...
    try:
        ...
        print(f"\n[종료 후 메모리 상태]")
        print(f"  사용 중: {mem_after.used / (1024**3):.2f} GB ({mem_after.percent}%)")
        freed = (mem_before.used - mem_after.used) / (1024**3)
        if freed > 0:
            print(f"  해제된 메모리: {freed:.2f} GB")
            
    except Exception as e:
        print(f"자원 정리 중 오류 발생: {e}")
