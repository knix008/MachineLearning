...
# https://prompthero.com/prompt/941604fcb30-flux-flux-11-pro-ultra-a-hyper-realistic-ultra-detailed-full-body-front-view-depiction-of-keeley-hazell-with-no-expression-her-dark-brown-hair

# Default values for each prompt section
DEFAULT_SUBJECT = "A high-quality, photorealistic full-body portrait of a young Korean woman with a soft, idol aesthetic, wearing an elegant white dress in a bright, sunlit living room."

DEFAULT_FACE = "She has a fair, clear, porcelain complexion. She is wearing striking bright blue contact lenses that contrast with her dark hair. Her expression is innocent and curious, looking directly at the camera with no smile."

DEFAULT_POSE_HEAD = "Long, straight jet-black hair with thick, straight-cut bangs (fringe) that frame her face."

DEFAULT_POSE_BODY = "She is standing gracefully in the center of a bright living room. Her body is slightly angled toward the camera, weight shifted gently to one side, creating a soft and elegant posture."

DEFAULT_POSE_ARM = "Both arms hanging naturally down. One arm with the elbow slightly bent, creating a relaxed and natural curve. The other arm hanging straight and loose at her side."

DEFAULT_POSE_HAND = "Both hands relaxed and open at her sides. The hand of the slightly bent arm rests softly near her hip."

DEFAULT_POSE_LEG = "Legs posed in a model-like stance: one leg slightly crossed in front of the other, with the weight shifted onto the back leg, creating a natural elongating line that emphasizes the thigh slit of the dress."

DEFAULT_POSE_FOOT = "Feet positioned with one foot slightly forward and angled outward, in a classic runway model pose, accentuating the white stiletto heels."

DEFAULT_HEADWEAR = ""

DEFAULT_TOP = "Elegant white one-piece dress with a graceful V-neckline and delicate lace trim along the neckline and hem. The fabric is soft chiffon, lightweight and flowing, with a fitted bodice that drapes beautifully over her figure."

DEFAULT_BOTTOM = "The dress flows into a gently flared A-line skirt falling to just above the knee, featuring a deep slit on one side that reveals her bare thigh. The slit opens naturally with her pose, creating an elegant and alluring silhouette."

DEFAULT_LEGWEAR = "White over-knee socks with a clean, crisp finish, adding a soft and innocent charm to the outfit."

DEFAULT_FOOTWEAR = "White stiletto high heels with a sleek, elegant design that complements the white dress seamlessly."

DEFAULT_ARMWEAR = ""

DEFAULT_SETTING = "Bright modern living room, floor-to-ceiling windows with sheer white curtains, light oak floor, white sofa, potted plants, bokeh background. No bed."

DEFAULT_LIGHTING = "Warm, golden natural sunlight from large windows, soft shadows, bright and sun-drenched atmosphere."

DEFAULT_CAMERA = "8k resolution, cinematic soft focus, photorealistic, gravure photography style, detailed fabric textures."

DEFAULT_NEGATIVE_PROMPT = "extra hands, extra legs, extra feet, extra arms, paintings, sketches, worst quality, low quality, normal quality, lowres, monochrome, grayscale, skin spots, acnes, skin blemishes, age spot, backlight, mutated hands, poorly drawn hands, blurry, bad anatomy, bad proportions, extra limbs, disfigured, missing arms, extra legs, fused fingers, too many fingers, unclear eyes, bad hands, missing fingers, extra digit, bad hands, missing fingers"
...
def get_device_label(device: str) -> str:
    """Return a device label including GPU model name."""
...
def load_model(device_name=None):
    """Load and initialize the Z-Image-Turbo model with optimizations."""
...
def generate_image(
    ...
    try:
        ...
        try:
            ...
            if tokenizer is not None:
                ...
                if clipped > 0:
                    ...
                    truncated_text = tokenizer.decode(raw_ids[max_len:], skip_special_tokens=True)
                    print("[잘린 텍스트]")
        ...
        filename = (
            f"{script_name}_{timestamp}_{device_label}_{int(width)}x{int(height)}"
...
def main():
    ...
    with gr.Blocks(title="Z-Image-Turbo Text-to-Image Generator") as interface:
        gr.Markdown("# Z-Image-Turbo Text-to-Image Generator")
        gr.Markdown(
            f"Tongyi-MAI/Z-Image-Turbo 모델을 사용하여 텍스트에서 이미지를 생성합니다."
        ...
        with gr.Row():
            ...
            # Right column: Parameters + Image generation
            with gr.Column(scale=1):
                gr.Markdown("### 파라미터 설정")
                ...
                with gr.Row():
                    guidance_scale = gr.Slider(
                        ...
                        step=0.1,
                        value=0.0,
                        info="Turbo 모델은 0.0 권장. 높을수록 프롬프트에 충실.",
                    )
                    num_inference_steps = gr.Slider(
                        ...
                        step=1,
                        value=9,
                        info="Turbo 모델 권장: 8-9. 실제 DiT forward는 steps-1회.",
                    )
