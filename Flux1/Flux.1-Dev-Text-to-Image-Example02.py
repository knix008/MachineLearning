...
SUBJECT = "A full body photography of a beautiful skinny young Korean woman standing by a luxury hotel swimming pool wearing a thong bikini, bare feet on the poolside deck."
...
HEAD = "Head held upright, tilted very slightly."

HEADWEAR = ""

BODY = "Body facing directly forward toward the camera, standing still, hip slightly tilted to create a gentle S-curve at the waist, posture elegant and natural."

TOP = "Wearing a tiny white triangle bikini top, minimal coverage, thin string ties at the neck and back, bare midriff fully exposed."

ARM = "Both arms relaxed and hanging naturally at the sides."

ARMWEAR = ""

HAND = "Both hands relaxed with fingers lightly extended, hanging naturally at the sides."

BOTTOM = "Wearing a tiny white thong bikini bottom, minimal coverage, bare hips fully exposed, string ties at the hips."

LEG = "One leg slightly bent at the knee while the other leg is straight supporting most of the body weight, creating a natural hip curve, long slender legs clearly visible."

LEGWEAR = ""

FOOT = "Both feet close together, touching side by side, flat on the poolside deck, bare feet."
...
LIGHTING = "Brilliant direct sunlight from the front, entire body from head to bare feet completely and evenly flooded with bright sunlight, skin glowing and luminous, no shadows, no backlighting."

POSITIVE = "8k, high quality, photorealistic, razor sharp focus, perfect anatomy, ten fingers. ten toes, beautifull fingers, beautiful toes."

NEGATIVE = "Blurry, out of focus, soft focus, hazy, low sharpness, grainy, low quality, deformed, bad anatomy, extra limbs, ugly, watermark, text, signature, extra fingers, extra toes, feet cropped."
...
def combine_t5_prompt_sections(
    ...
    순서: 외모/의상(위→아래) → 포즈 → 배경 → 기술적 설정
    subject → face → headwear → top → armwear → bottom → legwear → footwear
    → head → body → arm → hand → leg → foot → setting → camera → lighting → positive
    """
    sections = [
        subject,
        face,
...
DEFAULT_COMBINED_T5_PROMPT = combine_t5_prompt_sections(*DEFAULT_PROMPT_SECTIONS)
DEFAULT_COMBINED_CLIP_PROMPT = format_clip_prompt(SUBJECT)
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                gr.Markdown(
                    ...
                    "- **T5**: Subject → 얼굴/외모 → 의상(위→아래) → 포즈 → Setting → Camera → Lighting → Positive 순.\n"
                ...
                with gr.Accordion("프롬프트 섹션 (Subject · 외모 · 의상 · 포즈 · 배경 · 카메라 · 조명 · Positive)", open=True):
                    ...
                    positive_box = gr.Textbox(
                        ...
                        info="T5 프롬프트 맨 뒤에 포함됩니다. CLIP에는 포함되지 않습니다.",
                    )
                ...
                with gr.Accordion("CLIP 프롬프트 (Subject만)", open=False):
                    prompt_clip = gr.Textbox(
                        ...
                        lines=3,
                        interactive=True,
                        info="Subject만 자동 반영됩니다. Positive는 T5에만 포함됩니다.",
                    )
                with gr.Accordion("T5 프롬프트 (전체 섹션 결합)", open=False):
                    prompt_t5 = gr.Textbox(
                        ...
                        interactive=True,
                        info=(
                            "Subject → 얼굴/외모 → 의상(위→아래) → 포즈 → Setting → Camera → Lighting → Positive 순으로 자동 결합. "
                ...
                # 순서: subject, face, headwear, top, armwear, bottom, legwear, footwear,
                #        head, body, arm, hand, leg, foot, setting, camera, lighting, positive
                prompt_sections = [
                    prompt_subject,
                    prompt_face,
