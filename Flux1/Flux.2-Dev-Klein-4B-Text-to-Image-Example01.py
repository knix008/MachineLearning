...
DEFAULT_SUBJECT = "A highly realistic, 4k, high-quality, high resolution, beautiful skinny Korean woman."

DEFAULT_APPEARANCE = "Long wavy jet-black hair reaching her waist. Beautiful face with big eyes, small nose, full lips, and flawless skin. Young adult, around 20 years old."

DEFAULT_POSE = "Standing and looking at the viewer."

DEFAULT_OUTFIT = "Wearing simple string type micro tiny high-cut red bikini. "

DEFAULT_SETTING = "Solid orange backdrop."

DEFAULT_LIGHTING = "Studio lighting, mimics a large aperture f/1.4, style raw."

DEFAULT_CAMERA = "ar 9:16, high resolution."
...
def load_model(device_name=None):
    """Load and initialize the Flux2-Klein-4B model with optimizations."""
    ...
    if DEVICE == "cuda":
        ...
        pipe.enable_model_cpu_offload()
        pipe.enable_attention_slicing()
        print("메모리 최적화 적용: model CPU offload, attention slicing (CPU)")
    elif DEVICE == "mps":
        pipe.enable_attention_slicing()
...
def main():
    ...
    with gr.Blocks(
        title="Flux2-Klein-4B Text-to-Image Generator",
    ) as interface:
        gr.Markdown("# Flux2-Klein-4B Text-to-Image Generator")
        gr.Markdown(
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                prompt_pose = gr.Textbox(
                    ...
                    value=DEFAULT_POSE,
                    lines=2,
                    placeholder="예: standing, looking at the viewer",
                ...
                prompt_outfit = gr.Textbox(
                    ...
                    lines=2,
                    placeholder="예: red bikini, high heels",
                ...
                prompt_setting = gr.Textbox(
                    ...
                    lines=2,
                    placeholder="예: solid orange backdrop, studio",
                ...
                prompt_lighting = gr.Textbox(
                    ...
                    lines=2,
                    placeholder="예: studio lighting, f/1.4, style raw",
                ...
                prompt_camera = gr.Textbox(
                    ...
                    lines=2,
                    placeholder="예: ar 9:16, high resolution",
            ...
            with gr.Column(scale=1):
                ...
                with gr.Row():
                    guidance_scale = gr.Slider(
                        ...
                        step=0.1,
                        value=1.0,
                        info="프롬프트 준수도. 낮으면 창의적, 높으면 정확. 권장: 1.0",
                    )
                    num_inference_steps = gr.Slider(
                        ...
                        step=1,
                        value=4,
                        info="생성 단계 수. 높으면 품질 향상, 시간 증가. 권장: 4",
                    )
