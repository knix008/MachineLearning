...
def load_model(device_name=None):
    ...
    print("FLUX.2-dev: Mistral3 단일 VLM 인코더를 사용합니다.")
...
def generate_image(
    ...
    try:
        ...
        base_filename = (
            ...
            f"_n{int(num_images_per_prompt)}_msl{int(max_sequence_length)}"
        )
...
def main():
    ...
    with gr.Blocks(title="FLUX.2-dev Text-to-Image Generator") as interface:
        gr.Markdown("# FLUX.2-dev Text-to-Image Generator")
        gr.Markdown(
            ...
            with gr.Column(scale=1):
                ...
                with gr.Row():
                    guidance_scale = gr.Slider(
                        ...
                        info="FLUX.2-dev 권장: 3.5~8.",
                    )
                    num_inference_steps = gr.Slider(
                        label="추론 스텝",
                        minimum=10, maximum=50, step=1, value=28,
                        info="FLUX.2-dev 권장: 25~40.",
                    )
                with gr.Row():
                ...
                with gr.Row():
                    ...
                    max_sequence_length = gr.Slider(
                        label="최대 시퀀스 길이",
                        minimum=256, maximum=2048, step=256, value=1024,
                        info="Mistral3 인코더 최대 길이. FLUX.2-dev 권장: 1024~2048.",
                    )
                with gr.Row():
