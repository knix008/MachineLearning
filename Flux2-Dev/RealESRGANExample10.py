...
class ProgressCapture:
    """stdout/stderr 출력을 가로채서 타일 진행률을 Gradio progress에 반영합니다."""

    def __init__(self, original, progress_fn, start=0.3, end=0.8):
    ...
    def write(self, text):
        ...
        if match:
            ...
            if total > 0 and 0 < current <= total:
                ...
                self.progress_fn(val, desc=f"타일 처리 중... ({current}/{total})")
        return len(text)
...
def enhance_image(
    ...
    capture_out = ProgressCapture(sys.stdout, progress, 0.3, 0.8)
    capture_err = ProgressCapture(sys.stderr, progress, 0.3, 0.8)
