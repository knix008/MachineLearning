...
DEFAULT_PROMPT = "4k, A highly realistic, high-quality photo of a beautiful Instagram-style cute skinny korean girl. She has black, medium-length hair that reaches her shoulders, tied back in a casual yet stylish manner. Her eyes are hazel, with a natural sparkle of happiness as she smiles, wearing a cute red bikini. The image should capture her in a full-body shot, with perfect anatomy, including precise details in her eyes and teeth. Her skin should appear natural, with visible pores, avoiding an overly smooth or filtered look, to maintain a lifelike, The overall atmosphere is bright and joyful, reflecting the sunny, relaxed vacation mood."
...
def load_model():
    ...
    if DEVICE == "cuda" | DEVICE == "cpu": # CUDA or CPU?
        pipe = pipe.to(DEVICE)
...
def generate_image(
    prompt, height, width, guidance_scale, strength, num_inference_steps, seed
):
    ...
    try:
        ...
        if (
            hasattr(pipe, "__call__")
            and "strength" in pipe.__call__.__code__.co_varnames
        ):
