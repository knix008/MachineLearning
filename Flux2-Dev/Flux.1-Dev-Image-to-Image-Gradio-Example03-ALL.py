...
DEFAULT_PROMPT = "Make her wearing a red lingerie and hands up. Perfect anatomy, perfect fingers, perfect toes, perfect proportions, detailed face, intricate details, high quality, 8K, 4k, sharp focus, masterpiece, best quality, ultra-detailed, cinematic lighting"
...
def load_model(device=None):
    ...
    if device_type == "cuda":
        ...
        print(
            f"메모리 최적화 적용: model CPU offload, attention slicing ({DEVICE})"
        )
...
def main():
    ...
    default_label = next(
        label for label, value in AVAILABLE_DEVICES if value == DEVICE
    )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                input_image = gr.Image(
                    label="입력 이미지", type="pil", height=400, value=DEFAULT_TEST_IMAGE
                )
