...
DEFAULT_PROMPT = "She is wearing a red high legged bikini and  red fabric bunny ears with white lace inner lining. 4k, ultra-detailed texture, perfect anatomy, high definition, fashion vibe."
...
def load_model():
    ...
    if DEVICE == "cuda" or DEVICE == "cpu":
        ...
        print(f"알 수 없는 디바이스 유형: {DEVICE}. 최적화가 적용되지 않을 수 있습니다.")
        exit(1)
...
def generate_image(input_image, prompt, negative_prompt, height, width, guidance_scale, num_inference_steps, seed, progress=gr.Progress(track_tqdm=True)):
    ...
    try:
        ...
        print(f"이미지 편집 중... (steps: {steps}, seed: {int(seed)})")
        if negative_prompt:
        ...
        output_path = f"{base_name}_{timestamp}_{DEVICE.upper()}_step{steps}_seed{int(seed)}.png"
        image.save(output_path)
        print(f"이미지 저장됨: {output_path}")
        print(f"총 소요 시간: {elapsed:.1f}초")

        progress(1.0, desc="완료!")
        return image, f"✓ 이미지 편집 완료! ({elapsed:.1f}초) 저장됨: {output_path}"
