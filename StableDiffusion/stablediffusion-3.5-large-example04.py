...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    gr.Markdown(
        """
        # 🎨 Stable Diffusion 3.5 large : 이미지 변환기
        이미지를 업로드하고, 어떻게 바꿀지 프롬프트로 알려주세요!
        """
    )
    with gr.Row():
        with gr.Column():
            image_input = gr.Image(type="numpy", label="🖼️ 원본 이미지", height=500)
            prompt_input = gr.Textbox(
                label="📝 프롬프트",
                placeholder="예: A vibrant oil painting of a futuristic city",
                info = "이미지를 어떻게 바꿀지 설명하는 텍스트를 입력하세요.",
                value= "ultra detail, high quality, pores visible, 8k resolution, masterpiece, best quality, realistic, cinematic lighting, intricate details, ultra high resolution, skin texture details",
            )
            negative_prompt_input = gr.Textbox(
                label="🚫 네거티브 프롬프트 (선택)",
                placeholder="예: blurry, low quality, watermark",
                info="생성에서 피하고 싶은 요소를 입력하세요.",
                value= "low quality, worst quality, normal quality, blurry, duplicate, watermark, text, error, cropped, out of frame, extra fingers, fewer fingers, missing fingers, bad anatomy, bad hands, missing arms, missing legs, extra arms, extra legs, mutated hands and fingers, poorly drawn hands and fingers",
            )
