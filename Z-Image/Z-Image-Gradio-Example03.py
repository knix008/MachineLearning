...
DEFAULT_SUBJECT = "An image of a young beautiful skinny tall Korean woman with idol aesthetics."

DEFAULT_NEGATIVE = ""

DEFAULT_APPEARANCE = "Fair, clear complexion with no makeup, bare natural skin. Blue contact lenses contrasting with dark hair. Long wavy jet-black hair with thick straight-cut bangs. Sultry, confident expression, lips slightly parted, gazing directly at the camera."

DEFAULT_POSE = "Standing on the beach, full body visible from head to feet. One hand resting on her hip, the other gently touching her hair."

DEFAULT_OUTFIT = "Wearing a very tiny light blue string bikini top with barely-there triangle cups and a very tiny light blue G-string bikini bottom."

DEFAULT_ENV = "Tropical beach. White sandy shore with gentle waves in the background. Clear turquoise ocean water. Bright sunny day with a few soft clouds. Warm golden sunlight casting gentle shadows. Light ocean breeze."

DEFAULT_STYLE = "Photorealistic, ultra-realistic 8K resolution, tack-sharp focus, professional photography."

DEFAULT_COMPOSITION = "Full body shot, camera at eye level. Subject centered in frame, entire body from head to feet visible. Slight low angle to elongate the figure. Shallow depth of field, beach background softly blurred."


def combine_prompt_sections(subject, appearance, pose, outfit, env, style, composition):
    """Combine separate prompt sections into one final prompt string."""
    sections = [subject, appearance, pose, outfit, env, style, composition]
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                prompt_subject = gr.Textbox(
                    label="1. 인물 (Subject Specification)",
                    value=DEFAULT_SUBJECT,
                    lines=2,
                    placeholder="예: A young beautiful Korean woman.",
                    info="주요 피사체를 간략히 정의합니다. 인물 유형, 나이, 국적 등을 설명합니다.",
                )
                ...
                prompt_appearance = gr.Textbox(
                    ...
                    lines=3,
                    placeholder="예: Fair skin, blue contact lenses, long black wavy hair, straight-cut bangs...",
                    info="피부, 눈, 머리카락, 표정 등 인물의 외모를 묘사합니다.",
                )
                prompt_pose = gr.Textbox(
                    label="3. 포즈 (Pose)",
                    value=DEFAULT_POSE,
                    lines=3,
                    placeholder="예: Standing on the beach, one hand on hip, weight shifted to one leg...",
                    info="자세와 몸의 위치를 묘사합니다.",
                )
                prompt_outfit = gr.Textbox(
                    label="4. 의상 (Outfit)",
                    value=DEFAULT_OUTFIT,
                    lines=2,
                    placeholder="예: Wearing a tiny pink string bikini top and G-string bikini bottom...",
                    info="의상, 액세서리 등을 묘사합니다.",
                )
                prompt_env = gr.Textbox(
                    label="5. 환경/배경 (Environmental Context)",
                    value=DEFAULT_ENV,
                    lines=3,
                    placeholder="예: Tropical beach, sunny midday, warm golden sunlight...",
                    info="배경, 장소, 조명, 시간대 등 환경 전반을 설명합니다.",
                )
                prompt_style = gr.Textbox(
                    label="6. 시각적 스타일 (Visual Style)",
                    value=DEFAULT_STYLE,
                    lines=2,
                    placeholder="예: Photorealistic, shot on Leica M6, Kodak Portra 400 film grain...",
                    info="사진의 미적 처리 방식을 안내합니다. 카메라 기종, 필름, 렌더링 스타일 등을 설명합니다.",
                )
                prompt_composition = gr.Textbox(
                    label="7. 구도/프레이밍 (Composition)",
                    value=DEFAULT_COMPOSITION,
                    lines=2,
                    placeholder="예: Full body shot, eye level, shallow depth of field...",
                    info="카메라 앵글, 프레이밍, 피사계 심도 등 구도를 제어합니다.",
                )
