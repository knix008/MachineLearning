...
DEFAULT_SUBJECT = "A young beautiful Korean woman. Fair, clear complexion with no makeup, bare natural skin. Blue contact lenses contrasting with dark hair. Long wavy jet-black hair with thick straight-cut bangs. Wearing a very tiny light pink string bikini top and a very tiny light pink G-string bikini bottom. Standing on the beach in a sexy pose, full body visible from head to feet. One hand resting on her hip, the other gently touching her hair. Weight shifted to one leg, slightly arching her back. Sultry, confident expression, lips slightly parted, gazing directly at the camera."
...
def combine_prompt_sections(subject, env, style, composition):
    """Combine separate prompt sections into one final prompt string."""
    sections = [subject, env, style, composition]
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                prompt_subject = gr.Textbox(
                    label="1. 인물 명세 (Subject Specification)",
                    value=DEFAULT_SUBJECT,
                    lines=4,
                    placeholder="예: A young Korean woman, fair skin, black hair, wearing a bikini, lying on a sunbed...",
                    info="주요 피사체를 정의합니다. 인물 특징, 외모, 의상, 자세를 모두 포함합니다.",
                )
                ...
                prompt_env = gr.Textbox(
                    label="2. 환경/배경 (Environmental Context)",
                    value=DEFAULT_ENV,
                    lines=3,
                    placeholder="예: Rooftop pool, sunny midday, warm lighting, tropical plants...",
                    info="배경, 장소, 조명, 시간대 등 환경 전반을 설명합니다.",
                )
                prompt_style = gr.Textbox(
                    label="3. 시각적 스타일 (Visual Style)",
                    value=DEFAULT_STYLE,
                    lines=2,
                ...
                prompt_composition = gr.Textbox(
                    label="4. 구도/프레이밍 (Composition)",
                    value=DEFAULT_COMPOSITION,
                    lines=2,
                    placeholder="예: Bird's-eye view, overhead shot, full body visible...",
