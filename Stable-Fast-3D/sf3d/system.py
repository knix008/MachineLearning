...
def _patch_texture_baker_from_repo(installed_cls: Type[Any]) -> None:
    """Bind rasterize/interpolate from this repo if site-packages still has an old baker.py."""
    try:
        installed_path = Path(inspect.getfile(installed_cls)).resolve()
    ...
    if not repo_baker.is_file() or installed_path == repo_baker.resolve():
        return
    spec = importlib.util.spec_from_file_location(
        "_sf3d_repo_texture_baker_baker", repo_baker
    )
    if spec is None or spec.loader is None:
        return
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    repo_cls = getattr(mod, "TextureBaker", None)
    if repo_cls is None:
        return
    installed_cls.rasterize = repo_cls.rasterize  # type: ignore[method-assign]
    installed_cls.interpolate = repo_cls.interpolate  # type: ignore[method-assign]
    installed_cls.forward = repo_cls.forward  # type: ignore[method-assign]


try:
    from texture_baker import TextureBaker

    _patch_texture_baker_from_repo(TextureBaker)
except ImportError:
    import logging
