...
def _gradio_share_from_env() -> bool:
    """Match Gradio's GRADIO_SHARE=True behavior (public try.gradio / frp link)."""
    return os.environ.get("GRADIO_SHARE", "").strip().lower() == "true"


def _patch_gradio_client_json_schema() -> None:
    """gradio_litmodel3d emits JSON Schema with additionalProperties: false; gradio_client 1.x recurses into bool."""
    import gradio_client.utils as gc_utils

    _orig = gc_utils._json_schema_to_python_type

    def _safe(schema: Any, defs: Any) -> str:
        if not isinstance(schema, dict):
            return "Any"
        return _orig(schema, defs)

    gc_utils._json_schema_to_python_type = _safe  # type: ignore[method-assign]


_gradio_tmp = os.path.join(
...
demo.queue().launch(
    inbrowser=True,
    share=_gradio_share_from_env(),
    # Localhost HEAD probe often fails on Windows with proxy/VPN; browser still works.
    _frontend=False,
)
