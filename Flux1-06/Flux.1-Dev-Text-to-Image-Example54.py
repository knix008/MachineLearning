...
SUBJECT = "Full-body photo, slim young Korean woman, soft idol look, confident fashion model pose, summer day in Seoul."

FOOT = "Both feet pressed together, inner edges of feet touching, no gap between feet, both pointing forward; full feet and white sneakers in frame, uncropped."

LEG = "Weight fully on one straight supporting leg, other knee gently bent with foot still beside the supporting foot, subtle hip tilt from weight shift, clean leg silhouette visible."

FACE = "She has a fair, clear complexion. She is wearing striking bright blue contact lenses that contrast with her dark hair. Her expression is poised and confident with a soft editorial gaze toward the camera. She has long wavy voluminous jet-black hair with beautiful soft waves and curls, gently blown to one side by a soft breeze, a few strands floating across her face and shoulder."

BODY = "Body fully facing the camera, front-facing torso, shoulders square to lens, natural weight shift creating subtle hip tilt, clean body contour silhouette visible."

ARM = "Both arms relaxed and hanging naturally at sides, no hands on hips, elbows slightly soft, arms close to body."

HAND = "Both hands hanging naturally at sides, fingers relaxed and gently curved, no tension, graceful resting hand lines."

FOOTWEAR = "White canvas sneakers."

LEGWEAR = "Fitted blue jeans, ankles clear above shoes."

BOTTOM = "Slim blue jeans, medium wash, hem at ankle, feet unobscured."

TOP = "White sleeveless chiffon blouse, lace trim, semi-sheer with modest lining, bare shoulders."

HEADWEAR = ""

ARMWEAR = "Slim wristwatch on one wrist, delicate chain bracelet on the other wrist."

HEAD = "Chin slightly up, soft model head tilt, hair gently windswept to one side, light breeze motion visible in hair."

SETTING = "Bright summer Seoul street, trees, sun, pavement."

LIGHTING = "Even daylight, soft frontal light, face clearly lit."

CAMERA = "Wide full-body editorial, head to toe with margin below feet, sneakers visible, bokeh background."

POSITIVE = "8k, photorealistic, sharp focus, perfect anatomy, ten fingers, full body, feet and shoes visible, no extra limbs, no missing limbs, no deformed limbs, no distorted limbs, ten fingers, beautiful fingers, well formed fingers, no deformed hands, no extra legs."

NEGATIVE = "Blurry, low quality, deformed, bad anatomy, extra limbs, watermark, text, extra fingers, awkward pose, twisted torso, slouching, walking, running, sitting, cropped feet, feet out of frame, obscured shoes."
...
def count_t5_tokens(text: str) -> int:
    """T5 토큰 수. Flux 파이프라인의 tokenizer_2 사용(별도 HF 다운로드 없음). 모델 미로드 시 단어 수 추정."""
    text = (text or "").strip()
    if not text:
        return 0
...
def generate_image(
    ...
    try:
        ...
        # ---- Encode T5 ----
        # diffusers FluxPipeline: max_sequence_length > 512 는 ValueError (T5-XXL 포지션 한도).
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                gr.Markdown(
                    ...
                    f"- **T5 한도**: FLUX.1-dev는 최대 **{T5_MODEL_MAX_LENGTH}토큰**(모델 한도; 초과분은 잘림).\n"
                ...
                with gr.Accordion(
                    ...
                    negative_box = gr.Textbox(
                        label="19. 네거티브 프롬프트 (Negative · True CFG > 1.0 시 활성화)",
                        value=NEGATIVE,
                        lines=2,
            ...
            with gr.Column(scale=1):
                ...
                with gr.Row():
                    ...
                    max_sequence_length = gr.Slider(
                        ...
                        info=(
                            f"T5 인코딩 길이 상한(최대 {T5_MODEL_MAX_LENGTH}, FLUX.1-dev 모델 한도). "
                            "프롬프트가 더 길면 잘립니다."
                        ),
                    )
                with gr.Row():
