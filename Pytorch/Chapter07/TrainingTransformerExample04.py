...
def get_ptb_tokens_and_vocab():
    # ptb.words() returns all words in PTB
    tokens = list(ptb.words())
...
def save_model(model, vocab, path="transformer_ptb_model.pth"):
    checkpoint = {
...
def load_model(path="transformer_ptb_model.pth", device="cpu"):
...
def main(
    seq_len=32,
    batch_size=64,
    epochs=50, # Originally 100, reduced for faster training
    embed_dim=128,
    num_heads=4,
    num_layers=2,
    save_path="transformer_ptb_model.pth",
    do_train=True,
    do_generate=True,
    device="cuda" if torch.cuda.is_available() else "cpu",
):
    if do_train or not os.path.exists(save_path):
        data, vocab = get_ptb_tokens_and_vocab()
        n = len(data) - seq_len
        train_end = int(0.8 * n)
        val_end = int(0.9 * n)
